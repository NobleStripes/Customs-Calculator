...
describe('appApi.batchCalculate', () => {
  it('applies transit-specific global fees in the local fallback path', async () => {
    ...
    expect(result.data?.[0]?.breakdown?.globalFees?.totalGlobalTax).toBeCloseTo(1390)
    expect(result.data?.[0]?.costBase?.vatBase).toBeCloseTo(34740)
    expect(result.data?.[0]?.vat?.amount).toBeCloseTo(4168.8)
  })
  ...
  it('keeps transit totals distinct from consumption totals for the same shipment', async () => {
    ...
    expect(consumptionRow?.totalLandedCost).toBeCloseTo(38068.8)
    expect(transitRow?.totalLandedCost).toBeCloseTo(38908.8)
  ...
  it('returns computed batch amounts in PHP even when the input currency is not PHP', async () => {
    ...
    expect(row?.vat?.amount).toBeCloseTo(915.84)
    expect(row?.totalLandedCost).toBeCloseTo(8547.84)
  })
})
